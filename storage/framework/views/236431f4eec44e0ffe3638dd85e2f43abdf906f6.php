...
<div style="background: #515884;height: 195px;">
    ...
    <div class="text-center wr-top-20 single wr-padding-bottom-50 wr-padding-top-20">
        <h2 class="wr-top-30">￥<?php echo rmb($data['amount']); ?><span class="wr-meta">元</span></h2>
        <span style="color: #353537;font-size: 13pt">当前余额</span>
    </div>
</div>
<div class="row wr-top-115">
    <div class="wr-table-title-div <?php echo $table == 1 ? 'wr-bg-color-fff' : 'wr-color-b8b'; ?>" data-table="1">收支明细</div>
    <div class="wr-table-title-div <?php echo $table == 2 ? 'wr-bg-color-fff' : 'wr-color-b8b'; ?>" data-table="2">收入明细</div>
    <div class="wr-table-title-div <?php echo $table == 3 ? 'wr-bg-color-fff' : 'wr-color-b8b'; ?>" data-table="3">支出明细</div>
</div>
<div class="row" style="background: #fff;vertical-align: middle">
    ...
    <div class="wr-bottom-60 wr-content" style="padding: 0 6%;color: #353537;">
        <?php $__currentLoopData = $paginate['data']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="row wr-top-10 wr-bottom-20 wr-item">
                <span class="col-xs-8 wr-content-left"><?php echo $row->title; ?></span>
                <span class="col-xs-4 wr-content-right">￥<?php echo rmb($row->amount); ?>元</span>
            </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        <?php if(!isset($paginate['data']) || $paginate['data'] == []): ?>
            <div class="text-center" style="color: #B2B2B3">暂无数据</div>
        <?php endif; ?>
    </div>
</div>
<div class="row wr-paginate" style="color: #222222">
    <?php echo paging($paginate['current_page'], $paginate['last_page'], URL::to('/web/zpp/deposit'), Request::all()); ?>
...
<script>
    ...
    $(function () {
        var url = "<?php echo build_qs( Request::except(['table']) ); ?>";
        $('.wr-table-title-div').on('click', function () {
            window.location.href="/web/zpp/deposit?table=" + $(this).data('table') + '&' + url;
        })
    })
</script>
</html><?php /**PATH /mnt/hgfs/www/zpp-Server4.0/resources/views/web/user/deposit.blade.php ENDPATH**/ ?>
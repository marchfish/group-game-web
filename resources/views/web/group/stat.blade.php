...
<head>
    ...
    <script type="text/javascript" src="{!! URL::asset('forestage/public/blueimp-Templates-3.11.0/tmpl.js') !!}"></script>
</head>
<style>
    h1,h2,h3,h4,h5,p {
        margin: 0;
...
<div class="row wr-content-header">
    <div class="wr-top-25 clearfix wr-padding-05">
        <div id="date-picker-stat-company" class="text-left">
            <h3 class="wr-h3 {!! $date_month != '' ? 'wr-color-c3c' : '' !!}">{!! $data['date_show'] !!}</h3>
...
<div class="row wr-content-header wr-padding-05">
    ...
    <script>
        $(function () {
            ...
            $.ajax({
                type:"get",
                url:"/web/group/stat/income-trend?{!! build_qs( Request::except(['date_month','date_from','date_to']) ) !!}",
                data:{
...
<script>
    $(function () {
        ...
        $.ajax({
            type:"get",
            url:"/web/group/stat/order?{!! build_qs( Request::except(['date_month','date_from','date_to']) ) !!}",
            data:{
                date_from: '{!! $data['date_from'] !!}',
                date_to: '{!! $data['date_to'] !!}',
                date_month: '{!! $date_month !!}',
...
<script>
    $(function () {
        $.ajax({
            type:"get",
            url:"/web/group/stat/order1?{!! build_qs( Request::except(['date_month','date_from','date_to']) ) !!}",
            data:{
                date_from: '{!! $data['date_from'] !!}',
                date_to: '{!! $data['date_to'] !!}',
                date_month: '{!! $date_month !!}',
...
<script>
    $(function () {
        $.ajax({
            type:"get",
            url:"/web/group/stat/order2?{!! build_qs( Request::except(['date_month','date_from','date_to']) ) !!}",
            async:false, //异步
            data:{
                date_from: '{!! $data['date_from'] !!}',
                date_to: '{!! $data['date_to'] !!}',
